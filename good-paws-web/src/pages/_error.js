...
function Error({ statusCode }) {
    ...
    return (
        <Flex h={'100vh'} direction={'column'} justifyContent={'center'} alignItems={'center'}>
            {customError && <Text lineHeight={'24px'} size={'16px'} fontWeight={600} color={colors.primarylight}>{customError.code} error</Text>}
            <Heading mb={3} textAlign={'center'} maxW={'100%'} fontSize={{ base: '3xl', md: '4xl', lg: '6xl' }}>
                <Text>{customError.title}</Text>
            </Heading>
            <Text size={'20px'} p={6} fontWeight={400} lineHeight={'30px'} mb={6} color={colors.error}>{customError.description}</Text>
            <Link href="/">
