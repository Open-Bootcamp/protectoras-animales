...
function Error({ statusCode }) {
    ...
    return (
        <Flex h={'100vh'} direction={'column'} justifyContent={'center'} alignItems={'center'}>
            {customError && 
                <>
                    <Text lineHeight={'24px'} size={'16px'} fontWeight={600} color={'primarylight'}>{customError.code ? customError.code : '404'} error</Text>
                    ...
                    <Text size={'20px'} p={6} fontWeight={400} lineHeight={'30px'} mb={6} color={'error'}>{customError.description ? customError.description : 'Lo siento, no hemos encontrado lo que estabas buscando.'}</Text>
                    <Link href="/">
